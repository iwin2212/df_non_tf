...
def add2db(path, label):
    pass

def test_analysis(distance_metric, source=0, time_threshold=5, frame_threshold=5, no_label=15):
    ...
    # loading database
    embeddings = np.load("embedding1.npy", allow_pickle=True)
    ...
    while(True):
        try:
            t0 = time.time()
            ret, img = cap.read()
            fpstext = "FPS: " + str(1/(time.time() - t0)/10)[:2]
            if img is None:
                break
            raw_img = img.copy()
...
def test_stream(distance_metric='cosine', source=0, time_threshold=5, frame_threshold=5, no_label=15):
    """
    ...
    test_analysis(distance_metric, source=source, time_threshold=time_threshold, frame_threshold=frame_threshold, no_label=no_label)


# test_stream(time_threshold=0.02, frame_threshold=1, source="rtsp://admin:ECSIAQ@192.168.1.48:554")
